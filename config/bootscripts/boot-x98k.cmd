...
if test "${console}" = "display" || test "${console}" = "both"; then setenv consoleargs "console=tty1 console=tty0"; fi
...
# Calculate root partition number (boot partition + 1 for Armbian layout)
setexpr rootpart ${distro_bootpart} + 1

# Get PARTUUID of root partition for boot
if test "${devtype}" = "mmc"; then
	part uuid mmc ${devnum}:${rootpart} rootpart_uuid
	if test -n "${rootpart_uuid}"; then
		setenv rootdev "PARTUUID=${rootpart_uuid}"
	fi
fi
